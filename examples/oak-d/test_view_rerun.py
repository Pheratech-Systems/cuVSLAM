...
# Prints a self-connecting URL (viewer + data port already wired in).
import socket
...
# --- rerun: serve viewer (WEB_PORT) + data (GRPC_PORT) to the network, no local window ---
rr.init("cuVSLAM OAK-D")
if hasattr(rr, "serve_web"):
    rr.serve_web(open_browser=False, web_port=WEB_PORT, grpc_port=GRPC_PORT)
else:                                   # newer rerun split the API
    rr.serve_grpc(grpc_port=GRPC_PORT)
    rr.serve_web_viewer(open_browser=False, web_port=WEB_PORT)

ip = lan_ip()
url = f"http://{ip}:{WEB_PORT}/?url=rerun+http://{ip}:{GRPC_PORT}/proxy"
print("\n" + "=" * 78, flush=True)
print(f">> OPEN THIS ON YOUR COMPUTER:\n   {url}", flush=True)
print("=" * 78 + "\n", flush=True)
